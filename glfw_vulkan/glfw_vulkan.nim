...
import imguin/[cimgui, impl_glfw, impl_vulkan, simple]
import stb_image/read as stbi
import ../utils/[utils, setupFonts, vecs, togglebutton]

proc isWSL2(): bool =
  let (output, _) = osproc.execCmdEx("uname -r")
  return "microsoft" in output
...
# ============================================================
var
  g_Allocator: ptr VkAllocationCallbacks
  g_Instance: vulkan.VkInstance
...
proc getFrontendVersionString*(): string =
  fmt"GLFW v{$wglfw.getVersionString()}"
...
proc setupVulkanWindow(wd: ptr ImGui_ImplVulkanH_Window, surface: vulkan.VkSurfaceKHR, width, height: cint) =
  ...
  var res: VkBool32
  let res_vkresult = vkGetPhysicalDeviceSurfaceSupportKHR(g_PhysicalDevice, g_QueueFamily, surface, addr res)
...
proc main() =
  ...
  wglfw.windowHint(hClientApi.int32, oaNoApi.int32)
  when defined(windows):
      wglfw.windowHint(hVisible.int32, false.int32) # Avoid Window flickering at startup
  else:
    if isWSL2(): # WSL2 on Windows11
      discard
    else:
  ...
  when defined(windows):
    ...
    loadTileBarIcon(window, IconName)

  # Get Vulkan Instance Extensions from SDL
  var extensions: seq[string] = @[]
  var glfw_extensions_count: uint32 = 0
  let glfw_extensions = wglfw.getRequiredInstanceExtensions(addr glfw_extensions_count)
  if isNil glfw_extensions:
    echo fmt"Error: wglfw.getRequiredInstanceExtensions()"
    quit(1)
  for n in 0..<glfw_extensions_count:
    let ext = cast[ptr UncheckedArray[cstring]](glfw_extensions)
  ...
  let err = glfwCreateWindowSurface(g_Instance, window, g_Allocator, addr surface)
  check_vk_result(err)

  # ---- Create Framebuffers (Swapchain, etc.) ----
  var w, h: int32
  wglfw.getFrameBufferSize(window, addr w, addr h);
  ...
  ImGui_ImplGlfw_InitForVulkan(cast[ptr GlfwWindow](window), true)
  ...
  # Main loop
  #-----------
  while 0 == wglfw.windowShouldClose(window):
    wglfw.pollEvents()

    if 0 != wglfw.getWindowAttrib(window, wglfw.Iconified):
    ...
    wglfw.getFrameBufferSize(window, addr fb_width, addr fb_height);
    if (fb_width > 0) and
       (fb_height > 0) and
       (
    ...
    if showWindowDelay > 0:
      ...
      once: # Avoid flickering screen at startup.
        wglfw.showWindow(window)
  ### end while

  ## Cleanup
  let vkerr = vkDeviceWaitIdle(g_Device)
  checkVkResult(vkerr)
...
proc loadTileBarIcon*(win: wglfw.Window, iconName: string) =
  if iconName.fileExists:
    ...
    var img = wglfw.IconImageObj(width: w.int32, height: h.int32
      , pixels: cast[ptr uint8](pixels[0].addr))
    wglfw.setWindowIcon(win, 1, addr img)
  else:
