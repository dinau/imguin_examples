...
#--- zoomGlass
#--------------
proc zoomGlass*(textureID: var uint32, itemWidth:int, itemPosTop, itemPosEnd:Vec2 , capture = false) =
  ...
  if igBeginItemTooltip():
    ...
    let uv0 = Vec2(x: (region_x) / my_tex_w, y: (region_y) / my_tex_h)
    let uv1 = Vec2(x: (region_x + region_sz) / my_tex_w, y: (region_y + region_sz) / my_tex_h)
    let tint_col =  Vec4(x: 1.0f, y: 1.0f, z: 1.0f, w: 1.0f) #// No tint
    let border_col = Vec4(x: 0.22f, y: 0.56f, z: 0.22f, w: 1.0f) # Green
    igText(ICON_FA_MAGNIFYING_GLASS & " 4 x")
    let texRef = ImTextureRef(internal_TexData: nil, internal_TexID: textureID)
    igImage(texRef, Vec2(x: region_sz * zoom, y: region_sz * zoom), uv0, uv1) #, tint_col, border_col)
